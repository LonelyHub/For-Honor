...
;UI
	...
	Gui, Add, Text, x390 y65 w150 h20, [Pause/Resume]  -  Z
        Gui, Add, Text, x390 y85 w150 h20, [Parry/Button 1]  -  E
        Gui, Add, Text, x390 y105 w150 h20, [Parry/Button 2]  -  C
...
1- How To Use? 
2- READ ALL!
3- Open game and go into display settings, set feild of view on 81, contrast 55.
4- In graphics settings - disable shadows, Motion Blur, Ambient Occlusion, Dynamic Reflections, Dynamic Shadows 
5- keymapping(bind all in secondary slots unless say primary) bind arrows on movements , arrow up - forward, arrow down-backward, arrow left - left, arrow right - right,
6- For parry you need use right mouse button,
7- For Flips - Numpad 9 in secondary for light attack and fullblock,
8- For autoblock - Numpad 4 at left guard, Numpad 6 - at right guard, Numpad 8 at top guard 4 , unbind sprint form shift make it another key such as caps.
9- For Other characters you need full block - E(in primary key). gaurd  break numpad 5
10-When all keybinds ready you can start script, select characters on which you playing, tick in menu features which u want use(one per section but always have auto block and auto dodge ticked) 
11-Put ur resolution where X and Y - like x=1920 y=1080. (resolution in script gui and in game should be the same) and press    small ok button,
12-Now you can start script for that press Start Button, now go back in game and test it, all features working automaticly all    you can do thats parry - for that you need hold C or E, before enemy started an attack, when enemy doing heavy feint very  important let C or E go every time! for refresh parry and block direction. Script working in any game mode with borderless     windowed mode or fullscreen, - Not working in windowed mode! Best working on 1920x1080 fov 81 FPS - 120 (minimum) Monitor Hz   120 (minimum) my discord if u have questions XEAHT#3193 
13- Controller support-Steam and Uplay only.
14- For Uplay Add For Honor Starter Edition to your Steam library. Do not install yet. If you already started installing, then right click and select Uninstall.
Create a For Honor folder in steamapps\common, so it looks like ...\Steam\steamapps\common\For Honor
Copy or move every file from your uplay ...\ForHonor folder to ...\steamapps\common\For Honor
In Steam, go to your library and click Install. It should say "Discovering existing files for For Honor..." without downloading anything. Wait until it's finished. At the end of the process, it will download an extra 100mb or so
Launch the game on Steam
Uplay will come up and say "This will bind the following to you account [your name/email] / For Honor PTS Steam / For Honor Starter Edition" Click Continue. It won't replace your existing FH with starter edition or nullify your progress or anything.
The game will launch, and probably say that a patch has been applied. Have fun!
15- If you Uplay follow steam after you completed -14
16- Steam-controller layout , Browes Configs , recommended , offical for honor steam controller configuration , apply     config
17- Guard mode, Click Observer legacy keys and click C or E - Gaurd Direction , click action , legacy keys C or E -Y , remove ,     go back on Y , Toggle multi-button on , click revenge , cycle target , ececute 2.
18- Go in game settings controlls toggle lock controller.
)
}
return
...
AutoGB:
       if(Active21=1){
      PixelSearch,tx,ty,550,550,650,650,0xA9271F,5,Fast|RGB
      if(Errorlevel = 0) {
...
While, GetKeyState("C"){
      ...
      PixelSearch,px,py,x16,y16,x17,y17,0xFF9A8D,0,Fast|RGB
      if(ErrorLevel = 0){
if(Active9=1&&Warden=0){
Send, {RButton}
Sleep, 850
return
}
if(Active9=1&&Warden=1){
Send, {LButton}
Sleep, 500
Send, {LButton}
Sleep, 850
...
if(Active11=1&&Blackprior=0){
Send, {LButton}
Sleep, 1200
return
}
if(Active11=1&&Blackprior=1){
Send,{Up down}{Space Down}{Rbutton}{Space Up}{Up Up}{Rbutton}
Sleep, 850
return
}
if(Active12=1){
